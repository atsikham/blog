...
function openPost(postId, scrollToComments = false, skipAnimation = false) {
  ...
  // The floating close pill stays visible at all times so exit is always reachable.
  {
    ...
    let lastY   = 0;
    let ticking = false;
    // Only toggle after a meaningful delta to avoid flicker on tiny swipes.
    const HIDE_DELTA    = 12;  // px scrolled down before hiding
    const REVEAL_DELTA  = 12;  // px scrolled up before revealing
    const HIDE_OFFSET   = 80;  // don't hide until past this many px from top
    scrollArea.addEventListener("scroll", () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (!hidden && delta > HIDE_DELTA && y > HIDE_OFFSET) {
          topbar.classList.add("topbar-hidden");
          sidebar.classList.add("topbar-hidden");
        } else if (hidden && delta < -REVEAL_DELTA) {
...
function closeModal() {
  ...
  _pendingReplyToId = null;
  // Restore topbar + sidebar visibility for the next open
  document.querySelector(".modal-content-topbar")?.classList.remove("topbar-hidden");
  document.querySelector(".modal-sidebar")?.classList.remove("topbar-hidden");
  setHash("home");
  applyHomeSEO();
}
