...
NOTE: This post may be refined over time, but its foundational principles will remain constant.
...
* refreshing keeps the visitor on the same post
* the back button naturally closes an open modal
* there is no router library involved

Nav links use real `href` values — `/` for home, `#about` for the about page — so right-clicking and opening in a new tab works correctly without any extra JavaScript.
