...
NOTE: This post (and every post on tikho.me) now has a prerendered, crawlable URL for SEO and sharing: https://tikho.me/posts/1.html
It loads the same SPA and opens the post automatically, but Lighthouse and search engines can see the full `<title>`, `<meta>` tags and JSON-LD without executing JavaScript.
...
* refreshing keeps visitor on the same post
...
When a user click Reply, the form shows whom they are replying to.
